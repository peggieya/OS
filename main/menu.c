...
cmd_df(int nargs, char **args)
{
	...
	if(nargs != 3){
		...
		char unitsDig = args[1][0];

 		char tensDig = args[1][1];
	
		int bitIndex = 0;

		if (tensDig == '\0')
		{
			bitIndex = unitsDig - 48;
		}
		else
		{
			bitIndex = (unitsDig - 48) * 10 + (tensDig - 48);
		}
		...
		if (bitIndex < 13){
			...
			{
				bit_array[bitIndex - 1] = 1;
 
			}
			else if (strcmp("off", args[2]) == 0)
			{
				bit_array[bitIndex - 1] = 0;
